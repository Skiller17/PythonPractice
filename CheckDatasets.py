...
from WorkWithPredictAndRealData import *
...
def check_datasets(list_of_datasets: list, output_path: str, model_path: str):
    ...
    for dataset in tqdm(list_of_datasets, desc='Checked datasets', colour='white', ncols=100):
        sleep(1)
...
def check_images(path: str, output_path: str, no_match_dict: dict[str, int], classifier: BrandModelClassifier):
    ...
    for dataset in tqdm(os.listdir(abspath), desc='Checked internal datasets', colour='green', ncols=100):
        sleep(1)
        dataset_path = os.path.join(abspath, dataset)
        real_brands = os.listdir(dataset_path)

        for real_brand in tqdm(real_brands, desc='Checked brands', colour='blue', ncols=100):
            sleep(1)
